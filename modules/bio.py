def bio_scenarios_module(y, state, verbose):
    if sq.event(p_natural_bio(y - CURRENT_YEAR)):
        state['natural_pathogen'] = True
        if sq.event(p_natural_bio_is_catastrophe):
            if sq.event(p_xrisk_from_accidental_bio_given_catastrophe(y - CURRENT_YEAR)):
                if verbose:
