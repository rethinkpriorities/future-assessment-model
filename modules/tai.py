    ...
    if state['tai_type'] != 'abandoned' and state['tai_type'] != 'tool':
        ...
        if (sq.event(p_make_agent_tai) or state['tai_type'] == 'agent') and state['tai_type'] != 'tool':
            ...
            if sq.event(p_tai_intentional_misuse(state['war'])):
                ...
                if 0.5 * sq.event(p_alignment_solved(state['war'], y - CURRENT_YEAR, first_attempt=not state['tai'], verbose=verbose)):
                    if verbose:
                    ...
                    state['terminate'] = True; state['final_year'] = y
            elif sq.event(p_alignment_solved(state['war'], y - CURRENT_YEAR, first_attempt=not state['tai'], verbose=verbose)):
