...
def tai_scenarios_module(y, state, verbose):
    if state['tai'] and state['tai_type'] != 'abandoned' and state['tai_type'] != 'tool':
        ...
        if (sq.event(p_make_agent_tai) or state['tai_type'] == 'agent') and state['tai_type'] != 'tool':
            ...
            if sq.event(p_tai_intentional_misuse(state['war'])):
                # TODO: Parameterize the 0.5 (Who gets to TAI first affecting risk)
                # TODO: May depend on who the war is between?
                if 0.5 * sq.event(p_alignment_solved(state['war'], y - CURRENT_YEAR, first_attempt=not state['tai'], verbose=verbose) * p_alignment_deployment_safety_and_coordination(state['war'], y - CURRENT_YEAR, first_attempt=not state['tai'], verbose=verbose)):
                    if verbose:
                ...
                elif sq.event(p_full_tai_misalignment_averted):
                    state['averted_misalignment'] = True
                    if sq.event(p_tai_misalignment_averting_is_catastrophic):
                    ...
                    elif sq.event(p_full_tai_misalignment_averted_means_abandoned_tai):
                        if verbose:
                    ...
                    state['tai_type'] = 'agent'
                    if sq.event(p_tai_singleton_is_catastrophic):
                        if verbose:
                            print('...Singleton is catastrophic')
                        state['catastrophe'].append(state['category'])
                    state['terminate'] = True; state['final_year'] = y
            elif sq.event(p_alignment_solved(state['war'], y - CURRENT_YEAR, first_attempt=not state['tai'], verbose=verbose) * p_alignment_deployment_safety_and_coordination(state['war'], y - CURRENT_YEAR, first_attempt=not state['tai'], verbose=verbose)):
                if sq.event(p_subtle_alignment_solved):
                    if verbose:
                        print('{}: ...Achieved aligned TAI (aligned via work, {} attempt), happy future! :D'.format(y, '2nd+' if state['tai'] else 'first'))
                    state['category'] = 'aligned_tai'
                    state['tai_type'] = 'agent'
                    ...
                    state['tai_type'] = 'agent'
                    state['terminate'] = True; state['final_year'] = y
            elif sq.event(p_tai_aligned_by_default):
                if sq.event(p_subtle_alignment_solved_if_aligned_by_default):
                    if verbose:
            ...
            elif not p_alignment_deployment_safety_and_coordination(state['war'], y - CURRENT_YEAR, first_attempt=not state['tai'], verbose=verbose):
                if sq.event(p_full_tai_misalignment_averted):
                    state['averted_misalignment'] = True
                    if sq.event(p_tai_misalignment_averting_is_catastrophic):
                        # TODO: Right now this guarantees abandonment if catastrophe - revise?
                        if verbose:
                        ...
                        state['tai_type'] = 'agent'
                elif sq.event(p_tai_xrisk_is_extinction):
                    if verbose:
