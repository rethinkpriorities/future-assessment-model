def great_power_war_scenarios_module(y, state, variables, verbose):
    peace = y < state['peace_until'] if state['peace_until'] is not None else False
    
    if not state['war'] and (sq.event(variables['p_great_power_war_us_russia_without_nuke_first'](peace, y - variables['CURRENT_YEAR'], variables)) or
                             state['russia_nuke_first']):
        if verbose:
            print('{}: WAR!!! (US vs. Russia)'.format(y))
        state['war'] = True
        state['war_start_year'] = y
        war_length_ = int(round(~variables['war_length']))
    ...
    elif not state['war'] and (sq.event(variables['p_great_power_war_us_china'](peace, y - variables['CURRENT_YEAR'], variables)) or
    ...
    elif sq.event(variables['p_great_power_war_other'](peace, y - variables['CURRENT_YEAR'], variables)) and not state['war']:
        if verbose:
        ...
        peace_length_ = int(round(~variables['peace_length']))
