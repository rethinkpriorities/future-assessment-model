def check_for_double_dip_catastrophe(y, state, variables, verbose):
    if not state['terminate'] and variables['catastrophe_this_year']:
        if state['recent_catastrophe_year'] is None:
            state['recent_catastrophe_year'] = y
        else:
            prior_catastrophe_within_range = variables['n_catastrophes'] > 0 and y < (state['recent_catastrophe_year'] + variables['extinction_from_double_catastrophe_range'])
